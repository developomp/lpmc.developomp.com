...
overviewer.util = {
	...
	initialize: function () {
		...
		overviewer.coordBoxClass = L.Control.extend({
			...
			render: function (latlng) {
				...
				this.coord_box.innerHTML =
					"X " + Math.round(w_coords.x) + " Z " + Math.round(w_coords.z) + " (" + r_name + ")"
			},
			onAdd: function () {
		...
		overviewer.control = L.Control.extend({
			...
			onChange: function (ev) {
				...
				overviewer.layerCtrl = L.control
					.layers(base_layers, overlay_layers, { collapsed: false })
				...
				let selected_layer_name =
					overviewer.collections.mapTypes[selected_world] && overviewer.current_layer[selected_world]
		...
		overviewer.map.on("baselayerchange", function (ev) {
			...
			if (overviewer.current_layer[overviewer.current_world]) {
				...
				overviewer.map.setView(newMapCoords, overviewer.map.getZoom(), { animate: false })
			}
			...
			if (overviewer.current_world && overviewer.current_layer[overviewer.current_world]) {
				...
				if (tsc.markerCtrl) tsc.markerCtrl.remove()
			...
			if (ovconf.marker_groups) {
				...
				ovconf.markerCtrl = L.control
					.layers([], ovconf.marker_groups, { collapsed: false })
					.addTo(overviewer.map)
			}
					...
					for (var mkidx = 0; mkidx < markers[obj.path].length; mkidx++) {
						...
						for (var dbidx = 0; dbidx < markersDB[marker_entry.groupName].raw.length; dbidx++) {
							...
							if ("points" in db) {
								...
								let m_icon = L.divIcon({
									html: `<img class="ov-marker" src="${
										db.icon == undefined ? marker_entry.icon : db.icon
									}">`,
								})
								// Create marker
						...
						if (marker_entry.showIconInLegend) {
							layer_name_html =
								marker_entry.displayName +
								'<img class="ov-marker-legend" src="' +
								marker_entry.icon +
								'"></img>'
						} else {
	...
	fromWorldToLatLng: function (x, y, z, tset) {
		...
		// inverted
		var perPixel = 1.0 / (overviewerConfig.CONST.tileSize * Math.pow(2, zoomLevels))
	...
	updateHash: function () {
		...
		var coordinates = overviewer.util.fromLatLngToWorld(
			overviewer.map.getCenter().lat,
	...
	goToHash: function () {
		...
		if (!target_layer) {
			var default_tset_name = Object.keys(overviewer.collections.mapTypes[world_name])[0]
			target_layer = overviewer.collections.mapTypes[world_name][default_tset_name]
		}
		var ovconf = target_layer.tileSetConfig

		var latlngcoords = overviewer.util.fromWorldToLatLng(
			parseInt(coords[1]),
			parseInt(coords[2]),
			parseInt(coords[3]),
			ovconf
		...
		// clip zoom
		if (zoom > ovconf.maxZoom) zoom = ovconf.maxZoom
		if (zoom < ovconf.minZoom) zoom = ovconf.minZoom
		...
		if (ovconf.showlocationmarker) {
			...
			var locationm = L.marker(latlngcoords, { icon: locationIcon, title: "Linked location" })
