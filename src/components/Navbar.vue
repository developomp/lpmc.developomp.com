<template>
	<div class="navbar">
		<router-link to="/" class="logo"><img alt="LP logo" src="llama.png" />LPMC</router-link>
	...
	export default {
		methods: {
			borgerClick(ev: Event) {
				/* todo: find only once */
				let borger: HTMLElement = document.querySelector(".borger") || new HTMLElement()
				let nav: HTMLElement = document.querySelector(".navigation") || new HTMLElement()
				let navLinks: NodeListOf<Element> = document.querySelectorAll(".navigation a")

				borger.classList.toggle("toggle")
				nav.classList.toggle("active")
				navLinks.forEach((_link: Element, index: number) => {
					let link = (_link as HTMLElement)
